...
impl GptParser {
    ...
    pub fn parse_file<P: AsRef<std::path::Path>>(&mut self, path: P, sector_size: u32) -> Result<()> {
    ...
    fn decode_utf16le(bytes: &[u8]) -> Result<String> {
        ...
        UTF_16LE.decode(trimmed_bytes, DecoderTrap::Strict)
    ...
    pub fn print_summary(&self) {
        ...
        for (i, partition) in self.partitions.iter().enumerate() {
            ...
            println!("  Size: {} bytes", partition.size_in_bytes(self.sector_size as u64));
            println!("  Size: {:.2} MB", 
...
impl std::fmt::Display for PartitionEntry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{} (LBA {} - {}, {} sectors)", 
            self.name, self.first_lba, self.last_lba, 
            self.last_lba - self.first_lba + 1)
    }
}